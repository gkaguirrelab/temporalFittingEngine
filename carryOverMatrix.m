...
for i = 1:size(carryOver.storeAll,1)
    ...
    for j = 2:length(stimForRun)
        ...
        if isnan(carryOverMatSubAmp(stimForRun(j)==uniqueTempFreq,stimForRun(j-1)==uniqueTempFreq))
        ...
        counterMatrixSub(stimForRun(j)==uniqueTempFreq, ...
        stimForRun(j-1)==uniqueTempFreq) = ...
        counterMatrixSub(stimForRun(j)==uniqueTempFreq, ...
        stimForRun(j-1)==uniqueTempFreq)+1;
    end
